...
var LOGLEVEL = golog.INFO
var CLILOG = golog.ENABLED
var FILELOG = golog.ENABLED
var LOGPATH = "/tmp/sysmon-" + time.Now().Format("20060102-150405") + ".log"

func NewCPUAPI() *CPUAPI {
	if FILELOG == golog.ENABLED {
		err := golog.EnableLogFile(LOGPATH)
		if err != nil {
			panic(err)
		}
	}
	...
	capi := CPUAPI{CPU: NewCPU()}

	go capi.refreshService()

	return &capi
}

func (c *CPUAPI) refreshService() {
	for {
